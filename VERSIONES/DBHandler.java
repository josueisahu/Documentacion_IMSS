...
public class DBHandler extends SQLiteOpenHelper {
    ...
    // Database Name
    private static final String DATABASE_NAME = "DBMOVIL12";
...
//----------------------------------TABLE_Unidades--------------------------------------------------------------------------
            ...
            // you can use INSERT only
          sql = "INSERT  INTO " + TABLE_Unidades + " ( NumeroUnidad, Encargado ) VALUES ( ?, ? )";
		/*
		 * According to the docs http://developer.android.com/reference/android/database/sqlite/SQLiteDatabase.html
		 * Writers should use beginTransactionNonExclusive() or beginTransactionWithListenerNonExclusive(SQLiteTransactionListener)
		 * to start a transaction. Non-exclusive mode allows database file to be in readable by other threads executing queries.
...
//----------------------------------TABLE_DEPARTAMENTOS---------------------------------------------------------------------------------
        ...
        Datos = jObject.getJSONArray("detalleEquipos");
            // you can use INSERT only
            sql = "INSERT INTO " + TABLE_detalleEquipos + " ( idDetalleEquipo, numSerie,ip,macaddress, host, observaciones, extension," +
      ...
      //---------------------------------------------------------------------------
        ...
        Datos = jObject.getJSONArray("Equipos");
            sql = "INSERT INTO " + TABLE_Equipos + " ( numSerieEqui, numeroInventario,idModelo,status ) VALUES ( ?, ?, ?, ?)";
		/*
		 * According to the docs http://developer.android.com/reference/android/database/sqlite/SQLiteDatabase.html
		 * Writers should use beginTransactionNonExclusive() or beginTransactionWithListenerNonExclusive(SQLiteTransactionListener)
		 * to start a transaction. Non-exclusive mode allows database file to be in readable by other threads executing queries.
		 */
            ...
            sql = "INSERT INTO " + TABLE_Categorias + " ( clave,  descripcion ) VALUES ( ?, ?)";
...
//------------------------------------------------------------------------------------------------------
        ...
        Datos = jObject.getJSONArray("FotoConductores");
            sql = "INSERT INTO " + TABLE_FotoConductores + " ( id,descripcion, rendimiento ) VALUES (?,?, ?)";
            db.beginTransactionNonExclusive();
            // db.beginTransaction();
            stmt = db.compileStatement(sql);
        for (int i=0; i < Datos.length(); i++)
        ...
        Datos = jObject.getJSONArray("ImpresorasContadores");
            sql = "INSERT INTO " + TABLE_ImpresorasContadores + " ( folio,serie, idFotoconductor,contador ) VALUES (?,?, ?,?)";
            db.beginTransactionNonExclusive();
            // db.beginTransaction();
            stmt = db.compileStatement(sql);
        for (int i=0; i < Datos.length(); i++)
...
Log.e("MAMAM",usuariosList.toString());
// return contact list
        return usuariosList;
    }
/*
